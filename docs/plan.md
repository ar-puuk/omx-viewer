...
- [ ] Theme integration: VS Code's `--vscode-editor-background` etc. CSS variables in place of the custom dark/light toggle. **Deferred, not done** — shipped with the fixed dark theme for Task 6's functional-parity verification; revisit before considering the extension polished.
- [ ] Native affordance: command palette action ("OMX: Export Summary as CSV"), right-click "Open With" registration. **Right-click "Open With" works** (that's how the file was opened for testing) but the **command palette action is not implemented** — deferred.
