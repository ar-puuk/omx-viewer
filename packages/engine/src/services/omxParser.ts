...
/**
 * Validates HDF5 magic bytes from an already-in-memory byte array. Only the
 * first 8 bytes are inspected — callers that have a lazy source (e.g. a
 * browser File) should slice before reading, so a 700 MB file never needs
 * to be fully loaded just to fail this check.
 *
 * Host-agnostic by design: takes raw bytes, not a browser File — the
 * File-specific lazy-slice optimisation lives in apps/web's FileDropZone,
 * not here, since a VS Code webview has no File object to slice lazily.
 *
 * @param bytes - At least the first 8 bytes of the candidate file.
 * @returns     - true if magic bytes match, false otherwise.
 */
export function validateHDF5MagicBytes(bytes: Uint8Array): boolean {
