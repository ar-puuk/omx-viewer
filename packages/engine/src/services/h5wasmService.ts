 ...
 *   2. App calls openOMXFile(name, bytes) — validates, parses, stores h5File handle
 ...
 *
 * openOMXFile takes raw bytes, not a browser File — host-agnostic by design.
 * apps/web's App.svelte converts a File to bytes via file.arrayBuffer()
 * before calling in; apps/vscode-ext's webview will do the same with bytes
 * received via postMessage from the extension host.
 */
...
/**
 * Opens an OMX/HDF5 file from raw bytes, validates it, and returns the
 * parsed MatrixFile. Stores the h5wasm file handle in module scope for
 * subsequent slice calls.
 *
 * Host-agnostic: takes bytes already resident in memory, not a browser
 * File — callers are responsible for producing those bytes however is
 * natural for their host (file.arrayBuffer() in the browser; a postMessage
 * payload from the extension host in VS Code).
 *
 * Steps:
 ...
 *
 * @param name  - Filename, used for the virtual FS path and MatrixFile metadata.
 ...
 */
export async function openOMXFile(name: string, bytes: Uint8Array): Promise<MatrixFile> {
  // Step 1: Validate magic bytes before touching h5wasm
  if (!validateHDF5MagicBytes(bytes)) {
  ...
  const parsedFile = parseOMXFile(h5file, name)
