...
// ---------------------------------------------------------------------------
// Save a text blob — host-pluggable
// ---------------------------------------------------------------------------
//
// The default (Blob URL + programmatic <a download> click) only works in a
// real browser tab. It's a well-known VS Code webview limitation that this
// pattern doesn't reliably trigger a save inside a sandboxed webview iframe
// — confirmed in Task 6 (CSV export silently did nothing there). Unlike
// worker construction, there's no synchronously-catchable failure to
// self-heal from here (the click just silently no-ops), so this needs an
// explicit host-installed handler rather than a try/fallback. apps/vscode-ext
// installs one that posts the content to the extension host, which uses
// vscode.window.showSaveDialog() + vscode.workspace.fs.writeFile(). apps/web
// never calls setFileSaveHandler, so it keeps the default behavior.
// ---------------------------------------------------------------------------

export type FileSaveHandler = (content: string, filename: string, mimeType: string) => void

let fileSaveHandler: FileSaveHandler = (content, filename, mimeType) => {
...
/**
 * Saves text content as a file via the installed handler (default: browser
 * download; VS Code webview: routed to the extension host's save dialog).
 *
 * @param content  - The text content to save (e.g. a CSV string).
