<script lang="ts">
  // Component: MatrixTab — Individual tab button.
  ...
  const isActive = $derived(store.activeTabId === tab.id)
...
<div
  ...
  onkeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') store.setActiveTab(tab.id) }}
>
  ...
  <button class="close-btn" onclick={handleClose} onkeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') handleClose(e) }} aria-label={`Close ${tab.label}`} tabindex="-1">
...
<style>
  ...
  .close-btn { flex-shrink: 0; display: flex; align-items: center; justify-content: center; width: 16px; height: 16px; border-radius: var(--radius-sm); color: var(--color-text-muted); opacity: 0; transition: opacity var(--transition-fast), background-color var(--transition-fast); cursor: pointer; background: none; border: none; padding: 0; }
  .tab:hover .close-btn, .tab.is-active .close-btn { opacity: 1; }
  .close-btn:hover { background: var(--color-bg-overlay); color: var(--color-text-primary); }
</style>
