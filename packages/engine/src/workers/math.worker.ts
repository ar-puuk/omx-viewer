 ...
 * Aggregation (SUM, MEAN, MEDIAN, etc.) runs as a streaming h5wasm loop on
 * the main thread — see duckdbService.ts. There is no DuckDB dependency
 * anywhere in this codebase (duckdb.worker.ts was deleted as dead code).
 *
 * Message protocol:
 *   Main → Worker:
 *     {
