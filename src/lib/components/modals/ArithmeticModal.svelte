<script lang="ts">
  // Component: ArithmeticModal — Select two matrices, choose operator, compute result.
  // Sends data to math.worker.ts (NOT DuckDB) as Transferable Float64Arrays.

  import {
    file, fileMatrixIds, addEphemeralTab, addError
  ...
  const matrixNames = $derived(fileMatrixIds)

  let matrixA = $state(matrixNames[0] ?? '')
  let matrixB = $state(matrixNames[1] ?? matrixNames[0] ?? '')
  let op = $state<ArithmeticOp>('subtract')
  let isComputing = $state(false)

  const canCompute = $derived(matrixA && matrixB && !isComputing && !!file)

  const resultLabel = $derived(
    `${matrixA} ${ARITHMETIC_OP_SYMBOLS[op]} ${matrixB}`
  )
  ...
  async function handleCompute() {
    ...
    try {
      ...
      // Slice both full matrices from h5wasm
      const dataA = sliceFullMatrix(matrixA, nrows, ncols)
      const dataB = sliceFullMatrix(matrixB, nrows, ncols)

      // Send to math.worker.ts as Transferable buffers
      const result = await computeArithmetic(dataA, dataB, op)

      // Add as ephemeral tab
      addEphemeralTab(resultLabel, result)
      onclose()
    } catch (err) {
      const msg = err instanceof Error ? err.message : 'Arithmetic failed'
      addError(msg, 'error', true)
...
>
  ...
  <div class="modal animate-slide-up" role="document">
    ...
    <div class="modal-body">
      <p class="modal-desc">
        Compute element-wise arithmetic between two matrices.
        Result appears as a new temporary tab.
      </p>
      ...
      <!-- Operator selector -->
      <div class="form-group">
        <label class="form-label" for="op">Operation</label>
        <div class="op-pills" role="group" aria-label="Operation">
          {#each ARITHMETIC_OPS as o}
            <button
              ...
              onclick={() => { op = o }}
              aria-pressed={op === o}
              title={ARITHMETIC_OP_LABELS[o]}
            >
              {ARITHMETIC_OP_SYMBOLS[o]}
            </button>
          {/each}
        </div>
      </div>
