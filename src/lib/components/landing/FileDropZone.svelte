<script lang="ts">
  ...
  // Validates file type via magic bytes before dispatching onfile event.
  ...
  import { addError } from '../../state/matrixStore.svelte.js'
  ...
  async function handleFile(file: File) {
    ...
    if (file.size > FILE_SIZE_WARN_BYTES) {
      addError(
        `File is ${formatFileSize(file.size)} — processing may be slow on low-memory devices.`,
        'warning',
        true
      )
    }
    ...
    if (!valid) {
      addError('This file does not appear to be a valid HDF5/OMX file.', 'error', true)
      isValidating = false
      return
    }
  ...
  function onDragOver(e: DragEvent) {
    e.preventDefault()
    if (!disabled) isDraggingOver = true
  }
...
<style>
  .dropzone {
    ...
    border: 1.5px dashed var(--color-dropzone-border);
  ...
  .icon-wrap {
    ...
    transition: color var(--transition-normal), transform var(--transition-spring);
  }

  .dropzone:hover:not(.is-disabled) .icon-wrap,
  .dropzone.is-dragging-over .icon-wrap {
  ...
  @keyframes glow-pulse {
    ...
    50% {
      box-shadow: 0 0 24px 4px rgba(79, 142, 247, 0.2);
    }
  }
</style>
