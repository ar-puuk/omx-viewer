<script lang="ts">
  // Component: GridToolbar — Decimal toggle, CellNavigator, Arithmetic, Summary, CSV.
  ...
  async function handleExportCSV() {
    ...
    try {
      ...
      const csv = matrixSliceToCSV(data, sliceRows, ncols, 0, lookup, lookup, store.decimalPlaces)
      downloadTextFile(csv, `${tab.id}_export.csv`)
    } catch (err) { logger.error('GridToolbar: CSV export failed', err) }
  ...
  <div class="toolbar-group">
    ...
    <div class="decimal-pills" role="group" aria-label="Decimal places">
      {#each DECIMAL_OPTIONS as d}
        <button class="pill" class:is-active={store.decimalPlaces === d} onclick={() => store.setDecimalPlaces(d)} aria-pressed={store.decimalPlaces === d}>{d}</button>
      {/each}
    </div>
  </div>
  <div class="divider-v" aria-hidden="true"></div>
  <button class="compact-btn btn btn-ghost" class:is-active={store.compactNotation} onclick={() => store.toggleCompactNotation()} aria-pressed={store.compactNotation} title="Toggle compact notation">1.2M</button>
  <div class="divider-v" aria-hidden="true"></div>
...
<style>
  .toolbar { display: flex; align-items: center; gap: var(--space-6); height: var(--toolbar-height); padding: 0 var(--space-8); background: var(--color-toolbar-bg); border-bottom: 1px solid var(--color-border); flex-shrink: 0; overflow-x: auto; scrollbar-width: none; }
