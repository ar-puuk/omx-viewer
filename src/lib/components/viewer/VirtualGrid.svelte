<script lang="ts">
  // Component: VirtualGrid — Core 2D virtualized matrix renderer using TanStack Virtual.
  ...
  import { ROW_HEIGHT, COL_WIDTH, GRID_ROW_OVERSCAN, GRID_COL_OVERSCAN, DEFAULT_ROW_CHUNK_SIZE } from '../../utils/constants.js'
  import { logger } from '../../utils/logger.js'

  interface Props { scrollToCell?: (row: number, col: number) => void }
  ...
  const rowVirtualizer = $derived.by(() => {
    ...
    return createVirtualizer({ count: store.nrows, getScrollElement: () => scrollContainer!, estimateSize: () => ROW_HEIGHT, overscan: GRID_ROW_OVERSCAN })
  })

  const colVirtualizer = $derived.by(() => {
    if (!scrollContainer || store.ncols === 0) return null
    return createVirtualizer({ count: store.ncols, getScrollElement: () => scrollContainer!, estimateSize: () => COL_WIDTH, overscan: GRID_COL_OVERSCAN, horizontal: true })
  })

  // Expose scroll-to method
  $effect(() => {
  ...
  // Fetch visible rows on virtualizer change
  $effect(() => {
    const tab = store.activeTab
    if (!tab || !rowVirtualizer || store.ncols === 0) return
    const items = rowVirtualizer.getVirtualItems()
    if (items.length === 0) return

    const firstRow = items[0].index
    const lastRow  = items[items.length - 1].index + 1
    const chunkSize = DEFAULT_ROW_CHUNK_SIZE
    const chunkStart = Math.floor(firstRow / chunkSize) * chunkSize

    if (tab.cachedRows.has(chunkStart)) return

    try {
      if (!tab.isEphemeral) {
        sliceMatrixRows(tab.id, chunkStart, Math.min(chunkStart + chunkSize, store.nrows), store.ncols, tab.id, tab.cachedRows)
      }
    } catch (err) {
  ...
  function getCellValue(row: number, col: number): number | null {
    const tab = store.activeTab
    if (!tab) return null
    if (tab.isEphemeral && tab.ephemeralData) return tab.ephemeralData[row * store.ncols + col] ?? null
...
{#if scrollError}
  <div class="grid-error-state" role="alert">
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true"><circle cx="8" cy="8" r="7" stroke="currentColor" stroke-width="1.5"/><path d="M8 5v3M8 11h.01" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg>
    {scrollError}
  </div>
{:else}
  <div bind:this={scrollContainer} class="grid-scroll-container" role="grid" aria-rowcount={store.nrows} aria-colcount={store.ncols} aria-label="Matrix data grid">
    {#if rowVirtualizer && colVirtualizer}
      ...
      <div class="grid-header-row" style="width: {totalWidth + ROW_HEADER_WIDTH}px;">
        <div class="grid-corner-cell" style="width: {ROW_HEADER_WIDTH}px;">
          <span style="font-size: var(--font-size-xs); color: var(--color-text-muted); font-family: var(--font-mono);">{store.nrows}×{store.ncols}</span>
        </div>
        {#each virtualCols as vcol (vcol.index)}
          <div class="grid-col-header" class:is-pinned={store.pinnedCell?.col === vcol.index}
            style="width: {vcol.size}px; transform: translateX({vcol.start}px);"
            role="columnheader" aria-colindex={vcol.index + 1} title={getLabel(vcol.index)}>
            {getLabel(vcol.index)}
          </div>
        {/each}
      </div>
      ...
      <div class="grid-data-area" style="height: {totalHeight}px; width: {totalWidth + ROW_HEADER_WIDTH}px;">
        {#each virtualRows as vrow (vrow.index)}
          <div class="grid-row" class:is-pinned-row={store.pinnedCell?.row === vrow.index}
            style="transform: translateY({vrow.start}px); width: {totalWidth + ROW_HEADER_WIDTH}px;"
            role="row" aria-rowindex={vrow.index + 1}>
            <div class="grid-row-header" class:is-pinned={store.pinnedCell?.row === vrow.index}
              style="width: {ROW_HEADER_WIDTH}px;" role="rowheader" title={getLabel(vrow.index)}>
              {getLabel(vrow.index)}
            </div>
            {#each virtualCols as vcol (vcol.index)}
              ...
              <div
                ...
                role="gridcell" aria-colindex={vcol.index + 1} aria-selected={pinned} tabindex="-1"
                onclick={() => handleCellClick(vrow.index, vcol.index)}
                onkeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') handleCellClick(vrow.index, vcol.index) }}
              >
                {#if val !== null}{formatNumber(val, store.decimalPlaces, store.compactNotation)}{/if}
              </div>
            {/each}
          </div>
        {/each}
