<script lang="ts">
  ...
  async function handleFile(file: File) {
    ...
    // Lazily read only the first 8 bytes — avoids loading a 700 MB file
    // just to fail this check. Browser-specific optimisation; the engine's
    // validateHDF5MagicBytes() itself just takes whatever bytes it's given.
    const header = new Uint8Array(await file.slice(0, 8).arrayBuffer())
    const valid = validateHDF5MagicBytes(header)
